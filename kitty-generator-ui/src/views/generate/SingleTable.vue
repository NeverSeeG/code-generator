        ...
        <el-form ref="optionForm" class="optionForm" :inline="true" label-width="70px"
          ...
          <span>
            <el-form-item label="包名">
              <el-input v-model="generateModel.basePackage" placeholder="如：com.app.test">
          ...
          <span style="float:right; padding-right:20px;">
            <el-button size="small" type="primary" :disabled="disabledGenerateBtn" :loading="generateLoading" @click="generateHibernateCode">生成代码(hibernate)</el-button>
             <el-button size="small" type="primary" :disabled="disabledGenerateBtn" :loading="generateLoading" @click="generateCode">生成代码(mybatis)</el-button>
          </span>
        </el-form>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    tableSelectionChange(selections) {
      ...
      axios.post(this.baseUrl + "/getGenerateModel", params).then(res => {
        ...
        if (res.code == 200) {
          ...
          this.generateModel.outPutFolderPath = 'C:\\app'
    ...
    // 选择代码输出目录
    chooseBasePackage() {
      this.generateModel.basePackage = 'com.app.test'
    },
    // 选择代码输出目录
    /*chooseOutputFolder() {
      this.generateModel.outPutFolderPath = 'C:\\app\\output'
    },*/
    // 生成代码
    generateHibernateCode() {
      this.generateLoading = true;
      //axios.post(this.baseUrl + "/generateModels", this.generateModel).then(res => {
      axios.post(this.baseUrl + "/generateModelsToHibernate", this.generateModel).then(res => {
        res = res.data
        if (res.code == 200) {
